...
const { default: loopExtension } = await jiti.import('../../pi-loop-package/extensions/loop.ts');
...
test('messaging registration does not reset or govern the separate loop extension', async () => {
  ...
  const entries = []; const userMessages = []; let connections = 0;
  const pi = {
  ...
  const ctx = { mode: 'tui', ui: { notify: () => {}, setStatus: () => {} }, sessionManager: { getBranch: () => [] } };
