...
const { readConfig } = await jiti.import('../src/config.ts');
...
test('foreground bootstrap owns its child, authenticates clients and refuses lost initialized state', { timeout: 30000 }, async t => {
  ...
  const args = ['--experimental-strip-types', fileURLToPath(new URL('../src/broker.ts', import.meta.url)), '--port', String(port)];
  function start() {
